...
class Split:
    def __init__(self, workplane: cq.Workplane, use_cache: bool = False, max_dim: Optional[float] = None, tol: Optional[float] = None) -> None:
        self.workplane = self.initial_workplane = workplane
        self.use_cache = use_cache
        self.tol = tol
        self.pending_splits = list[list[cq.Face]]()
        self.max_dim = max_dim or workplane.findSolid().BoundingBox().DiagonalLength * 10
    ...
    def refresh(self):
        self.type_groups = CQLinq.groupByTypes(self.workplane, self.max_dim, self.tol, self.type_groups)
    ...
    def from_ratios(
        ...
        for ratio in ratios:
            ...
            nearest_face = cast(cq.Face, CQLinq.find_nearest(target, edge, shape_type="face"))
    ...
    def from_anchor(
        ...
        for anchor, angle in zip(anchors, angles):
            ...
            if until == "next":
                intersected_vertices = self.workplane.intersect(cq.Workplane(split_face)).vertices().vals()
                intersect_vertex = CQLinq.find_nearest(intersected_vertices, to_vec(anchor), snap_tolerance)
