...
def import_from_su2(file_path: str):
    ...
    for zone in su2_mesh.zones:
        ...
        for marker_label, su2_marker_element_types in zone.marker_types.items():
            marker_types[marker_label] = [
                ElementType[su2_marker_element_type.name]
                for su2_marker_element_type in su2_marker_element_types
            ]
...
def import_from_gmsh() -> Mesh:
    ...
    for element_type_value, grouped_element_tags, grouped_node_tags_concatted in zip(
        *grouped_concatted_elements
        ...
        if (
            element_type_value == ElementType.POINT.value
            or element_type_value == ElementType.LINE.value
        ):
            continue
        num_nodes = gmsh.model.mesh.getElementProperties(element_type_value)[3]
        group_elements = (
            np.array(grouped_node_tags_concatted, dtype=np.uint16).reshape(
                (-1, num_nodes)
            )
            - 1
        )
    ...
    for group_dim, group_tag in physical_groups:
        ...
        for entity in entities:
            ...
            marker_elements = (
                np.array(marker_node_tags_concatted, dtype=np.uint16).reshape(
                    (-1, node_count)
                )
                - 1
            )
            ...
            marker_types[marker_name] += [ElementType(marker_element_type)] * len(
                marker_elements
            )

    return Mesh(dim, elements, element_types, points, markers, marker_types)
