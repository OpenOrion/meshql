...
class UnstructuredBoundaryLayer(MultiEntityTransaction):
    ...
    def before_gen(self):
        ...
        extruded_bnd_layer = gmsh.model.geo.extrudeBoundaryLayer(
            dim_tags, [1] * self.num_layers, sizes, True
        )
...
class UnstructuredBoundaryLayer2D(MultiEntityTransaction):
    ...
    def before_gen(self):
        ...
        gmsh.model.mesh.field.setNumber(tag, "Quads", 1)
        gmsh.model.mesh.field.setNumber(tag, "Size", self.size)
        gmsh.model.mesh.field.setNumber(tag, "NbLayers", self.num_layers)
        gmsh.model.mesh.field.setNumber(tag, "BetaLaw", 1)
