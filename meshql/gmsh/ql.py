...
from meshql.gmsh.algorithm import MeshAlgorithm2DType, MeshAlgorithm3DType, MeshSubdivisionType, SetMeshAlgorithm2D, SetMeshAlgorithm3D, SetSubdivisionAlgorithm
...
class GmshGeometryQL(GeometryQL):
    ...
    def load(
        ...
        self.is_2d = CQExtensions.get_dimension(workplane) == 2
    ...
    def _setTransfiniteFaceAuto(
        ...
        for transfinite_group in self._transfinite_edge_groups:
            ...
            for cq_edge in transfinite_group:
                edge_num_nodes = int(
                    np.ceil((cq_edge.Length() / total_length) * max_nodes)  # type: ignore
                )
    ...
    def addTransaction(
        self, toTransaction: Callable[["GmshGeometryQL"], GmshTransaction]
    ):
        self._ctx.add_transaction(toTransaction(self))
        return self

    def setTransfiniteAuto(
        self, max_nodes: int, min_nodes: int = 1, auto_recombine: bool = True
    ):
        self.is_structured = True
        if CQExtensions.get_dimension(self._workplane) == 2:
            cq_faces = cast(
                Sequence[cq.Face], list(CQLinq.select(self._workplane, "face"))
            )
