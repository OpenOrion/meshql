...
class TestShapes:
    ...
    def test_generate_circle(self):
        ...
        assert coords.shape[0] == 100  # Default num_points
        ...
        assert coords_custom.shape[0] == 50
    ...
    def test_generate_naca4_airfoil_basic(self):
        ...
        assert trailing_edge_diff < 0.01, "Trailing edge should be approximately closed"

        # For NACA 0012 (symmetric), check that leading edge is at x=0 and max thickness
        min_x_idx = np.argmin(coords[:, 0])
        assert coords[min_x_idx, 0] == pytest.approx(0.0, abs=1e-8)
    ...
    def test_generate_naca4_airfoil_symmetric(self):
        ...
        all_y = coords[:, 1]
        assert np.max(all_y) > 0  # Has positive y values
        assert np.min(all_y) < 0  # Has negative y values

        # Check that the mean y coordinate is approximately zero for symmetric airfoil
        mean_y = np.mean(all_y)
        assert mean_y == pytest.approx(0.0, abs=0.01)
    ...
    def test_generate_naca4_airfoil_cambered(self):
        ...
        assert mean_y > 0, "Cambered airfoil should have positive mean y-coordinate"
    ...
    def test_generate_naca4_airfoil_cosine_sampling(self):
        ...
        assert coords_cosine.shape == coords_linear.shape
        ...
        assert np.mean(x_diffs_cosine) != np.mean(x_diffs_linear)
    ...
    def test_generate_naca4_airfoil_thickness(self):
        ...
        assert max_thickness > 0.05
        assert max_thickness < 0.15
